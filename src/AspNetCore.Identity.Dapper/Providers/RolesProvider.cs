...
{
    public async Task<IdentityResult> CreateAsync(ApplicationRole role, CancellationToken _)
    {
        var command = $"INSERT INTO [{databaseConnectionFactory.DbSchema}].AspNetRoles " +
    ...
    {
        var command = $"UPDATE [{databaseConnectionFactory.DbSchema}].AspNetRoles " +
        ...
        await using (var sqlConnection = await databaseConnectionFactory.CreateConnectionAsync())
        {
            ...
            if (role.Claims.Count > 0)
            {
                var deleteClaimsCommand = "DELETE " +
                                                  $"FROM [{databaseConnectionFactory.DbSchema}].RoleClaims " +
                                                  "WHERE RoleId = @RoleId;";
                ...
                var insertClaimsCommand = $"INSERT INTO [{databaseConnectionFactory.DbSchema}].RoleClaims (RoleId, ClaimType, ClaimValue) " +
    ...
    {
        var command = "DELETE " +
                               $"FROM [{databaseConnectionFactory.DbSchema}].AspNetRoles " +
                               "WHERE Id = @Id;";
